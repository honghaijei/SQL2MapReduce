    ...
    public static class Map extends Mapper<Object,Text, CompositeKey,Text>{
        ...
        public void setup(Context context) throws IOException, InterruptedException {
            ...
            if (table == 0) {
                ...
                if ((true)) {
                    context.write(new CompositeKey(Integer.parseInt(arr[3])), new Text(table + "|" + line));
                }            } else {
        ...
        public void reduce(CompositeKey key,Iterable<Text> values,Context context) throws IOException,InterruptedException{
            ...
            for (String[] arr1 : leftTuples) {
                for (String[] arr2 : rightTuples) {
                    context.write(t, new Text(arr1[0] + '|' + arr1[1] + '|' + arr1[2] + '|' + arr1[3] + '|' + arr1[4] + '|' + arr1[5] + '|' + arr1[6] + '|' + arr1[7] + '|' + arr2[0] + '|' + arr2[1] + '|' + arr2[2] + '|' + arr2[3]));
                }
            }
        }
        ...
        FileInputFormat.addInputPath(job, new Path("customer"));
        FileInputFormat.addInputPath(job, new Path("nation"));
        FileOutputFormat.setOutputPath(job, new Path("JOIN1"));
